...
export default async function TrackList({ tracks }: { tracks: Track[] }) {
  ...
  return (
    <section>
      <h2 className="text-xl font-bold text-foreground mb-4">{t('topTracks')}</h2>
        ...
        {tracks.map((track, i) => (
          <li key={`${track.name}-${i}`}>
            <a
              ...
              className="flex items-center gap-4 p-3 rounded-lg bg-card hover:bg-muted transition-colors group"
            >
              <span className="text-muted-foreground text-sm font-mono w-6 text-right shrink-0">
                {i + 1}
              </span>
