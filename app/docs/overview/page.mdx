...
2. **Locale detection** runs in the same proxy pass. The `Accept-Language` header is matched against supported locales (`en`, `es`, `no`, `it`, `fr`) and prefixed into the URL. next-intl then serves translated UI strings for every page in that locale.
