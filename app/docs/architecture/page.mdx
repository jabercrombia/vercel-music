...
flowchart TD
    ...
    subgraph B["Vercel Edge — proxy.ts"]
        B1[Read x-vercel-ip-country] --> B2[Map ISO to country slug]
        B3[Read Accept-Language] --> B4[Match locale]
        B2 & B4 --> B5["Rewrite / → /locale/trends/country"]
    end

    B --> C

    subgraph C["Next.js App Router"]
        C1["/[locale]/trends/[country]/page.tsx"] --> C2[Fetch top tracks + artists in parallel]
        C2 --> C3[Render page — ISR revalidate 3600s]
    end

    C --> D

    subgraph D["Vercel Edge Cache"]
        D1{Cache hit?}
        D1 -->|yes| D2[Serve stale HTML instantly]
        D1 -->|no| D3[Regenerate in background]
    end
`} />
